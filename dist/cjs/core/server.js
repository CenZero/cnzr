...
const middleware_engine_1 = require("./middleware-engine");
...
class CenzeroApp {
    constructor(options = {}) {
        ...
        this.myMainRouter = new router_1.Router();
        this.myMwEngine = new middleware_engine_1.MiddlewareEngine();
        this.myStaticHandler = new static_file_server_1.StaticFileServer(this.myAppConfig.staticDir);
        this.myViewEngine = new template_engine_1.TemplateEngine(this.myAppConfig.viewEngine, this.myAppConfig.viewsDir);
        this.myHttpServer = (0, http_1.createServer)(this.handleRequest.bind(this));
        this.myPluginMgr = new plugin_1.PluginManager(this);
        this.myErrHandler = new error_handler_1.ErrorHandlerManager(this.myAppConfig.useContext);
        ...
        if (this.myAppConfig.useFileRouting) {
            this.myFileBasedRouter = new file_router_1.FileRouter(this, this.myAppConfig.routesDir);
        }
    ...
    async enableFileRouting(routesDir = 'routes') {
        this.myFileBasedRouter = new file_router_1.FileRouter(this, routesDir);
   ...
   ╚═══════════════════════════════════════╝
    ...
    async handleRequest(req, res) {
        ...
        const cenzeroRes = response_helper_1.ResponseHelper.enhance(res);
        let ctx = null;
        // Increment request counter - personal statistics tracking
        this.myRequestCount++;
        try {
            // Parse request
            await request_parser_1.RequestParser.parseRequest(cenzeroReq);
            // Create context if using context mode
            ctx = this.myAppConfig.useContext ? new context_1.Context(cenzeroReq, cenzeroRes, this.mySessionConfig) : null;
