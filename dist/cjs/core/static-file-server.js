...
class StaticFileServer {
    constructor(defaultStaticDir) {
        this.myStaticMappings = new Map(); // lebih personal name
        this.servedFilesCount = 0; // tracking for fun
    ...
    addStaticPath(urlPath, fsDirectory) {
    ...
    async handleStatic(req, res) {
        ...
        for (const [urlPrefix, targetDir] of this.myStaticMappings) {
            if (requestPath.startsWith(urlPrefix)) {
                ...
                const absoluteFilePath = (0, path_1.join)(targetDir, relativePath);
        ...
        const defaultFilePath = (0, path_1.join)(this.defaultStaticPath, requestPath);
    ...
    async attemptFileServe(filePath, res, originalPath) {
        try {
            ...
            const mimeType = this.determineMimeType((0, path_1.extname)(filePath));
    ...
    determineMimeType(fileExtension) {
...
//# sourceMappingURL=static-file-server.js.map